...
namespace ik_constraint2_voxblox_distance_field{
  ...
  std::vector<cnoid::Vector3> getSurfaceVertices(cnoid::LinkPtr link, double resolution){
  ...
  bool VoxbloxDistanceFieldCollisionConstraint::computeDistance(const cnoid::LinkPtr A_link, const cnoid::LinkPtr B_link, double& distance, cnoid::Vector3& direction/*B->A*/, cnoid::Vector3& A_v, cnoid::Vector3& B_v) {
    if(A_link == nullptr ||
       B_link != nullptr ||
       field_ == nullptr){
      std::cerr << "[VoxbloxDistanceFieldCollisionConstraint::computeDistance] assertion failed" << std::endl;
    }

    // 別スレッドで上書きされてもいいようにコピー
    std::shared_ptr<voxblox::EsdfMap> field = this->field_;
    cnoid::Isometry3 fieldOrigin = this->fieldOrigin_;
    Eigen::Affine3d fieldOriginInv = fieldOrigin.inverse();
    ...
    cnoid::Isometry3 linkT = A_link->T();

    double min_dist = this->maxDistance_ + 1; // getUninitializedDistance() returns max_distance
    ...
    if(min_dist_grad_invalid >= this->maxDistance_ - static_cast<double>(field->voxel_size())/*epsilon*/){
      ...
      distance = std::min(static_cast<double>(field->voxel_size()), ((A_link->T() * this->prev_A_localp_) - this->prev_B_localp_).dot(this->prev_direction_));
    }else{
  ...
  std::shared_ptr<ik_constraint2::IKConstraint> VoxbloxDistanceFieldCollisionConstraint::clone(const std::map<cnoid::BodyPtr, cnoid::BodyPtr>& modelMap) const {
    std::shared_ptr<VoxbloxDistanceFieldCollisionConstraint> ret = std::make_shared<VoxbloxDistanceFieldCollisionConstraint>(*this);
    this->copy(ret, modelMap);
    return ret;
  }

  void VoxbloxDistanceFieldCollisionConstraint::copy(std::shared_ptr<VoxbloxDistanceFieldCollisionConstraint> ret, const std::map<cnoid::BodyPtr, cnoid::BodyPtr>& modelMap) const {
  ...
  void VoxbloxDistanceFieldCollisionConstraint::dummy() {
    qhT qh_qh;
    qhT* qh = &qh_qh;
    QHULL_LIB_CHECK
    qh_zero(qh, stderr);
