#ifndef IK_CONSTRAINT2_VOXBLOX_DISTANCE_FIELD_DISTANCE_FIELDCOLLISIONCONSTRAINT_H
#define IK_CONSTRAINT2_VOXBLOX_DISTANCE_FIELD_DISTANCE_FIELDCOLLISIONCONSTRAINT_H
...
namespace ik_constraint2_voxblox_distance_field{
  ...
  class VoxbloxDistanceFieldCollisionConstraint : public ik_constraint2::CollisionConstraint {
  public:
    class BoundingBox {
    public:
      ...
      void cacheParentLinkPose(){
        if(parentLink){
          ...
          worldPoseinv = cnoid::Affine3::Identity();
        }
      }
    protected:
      cnoid::Affine3 worldPoseinv;
    };
    ...
    void copy(std::shared_ptr<VoxbloxDistanceFieldCollisionConstraint> ret, const std::map<cnoid::BodyPtr, cnoid::BodyPtr>& modelMap) const;
  ...
  // link local frame
  std::vector<cnoid::Vector3> getSurfaceVertices(cnoid::LinkPtr link, double resolution = 0.02);
}
